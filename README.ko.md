...
- **캔버스 기반 다이어그램 에디터** — 팔레트에서 드래그 앤 드롭으로 노드 생성, 연결, 그룹핑, 레이어 관리
- **서브 인프라 드릴다운** — 노드 안에 서브노드 계층 구조 지원; 부모 노드는 서브노드 리소스를 자동 합산하며 전용 서브 뷰로 진입/복귀
- **레이어 표시/숨김 토글** — 눈 아이콘으로 레이어별 표시/숨김 전환; 숨긴 레이어는 캔버스, 선택, 리소스 계산에서 제외
- **120+ 클라우드 아이콘** — AWS, GCP, Azure, Kubernetes, 오픈소스 아이콘 내장 (아이콘 피커에서 드래그 가능)
- **k8s 리소스 대시보드** — 클러스터 하드웨어 설정, 노드별 CPU/메모리/디스크/GPU 리소스 관리 (항상 루트 레벨 전체 현황 표시)
- **인프라 완성도 체크** — 필수 구성요소 누락, 네트워크 연결, 리소스 초과 자동 감지
- **AI 다이어그램 생성** — 텍스트 설명으로 아키텍처 자동 생성 (Claude / OpenAI / Gemini / Ollama)
- **Mermaid 가져오기** — Mermaid 텍스트를 붙여넣으면 편집 가능한 도형으로 변환하거나 다이어그램 패널로 삽입
- **URL로 다이어그램 공유** — LZ-String 압축 해시 프래그먼트를 사용하여 링크 하나로 다이어그램 공유
- **다양한 내보내기** — JSON, Excel (리소스 시트 포함), PDF (표지+다이어그램+상세표), PowerPoint, PNG, SVG
- **벡터 · 고해상도 이미지** — SVG로 내보내면 확대해도 글자가 깨지지 않고, PNG는 1×~4× 배율 선택
- **PowerPoint 내보내기** — 그림이 아니라 편집 가능한 네이티브 도형과 텍스트로 생성
- **IaC 코드 내보내기** — k8s YAML, Terraform (HCL), Helm Values를 노드별 또는 전체 인프라로 생성
- **연결선 라우팅과 라벨** — 직각 / 직선 / 곡선 선 타입, 시작·끝 면 지정, Waypoint로 경로 제어, 라벨 위치와 텍스트 방향 조절
- **완전 오프라인 동작** — 라이브러리와 폰트를 모두 내장, 외부 CDN 없이 동작
- **자동 레이아웃** — 노드 자동 배치
...
`index.html` 파일 하나만 받아서 열어도 동작합니다. 이때 라이브러리와 글꼴은 CDN에서 받아오므로 인터넷이 필요합니다.

> **보안망(에어갭) 환경**: `index.html`과 `vendor/` 폴더를 함께 복사하면 인터넷 없이 전부 동작합니다. 다이어그램 라이브러리(Mermaid), 문서 내보내기 라이브러리, 웹폰트가 모두 `vendor/` 안에 들어 있어 외부 요청이 발생하지 않습니다.
>
> 파일을 브라우저로 직접 여는 경우(`file://`)에는 브라우저 보안 정책상 PDF용 한글 글꼴만 읽지 못해 영문 글꼴로 대체됩니다. 폴더를 간단한 로컬 웹서버로 띄우면(예: `python3 -m http.server`) 이 제약도 없어집니다.
...
| 동작 | 방법 |
|------|------|
| **노드 생성** | 왼쪽 팔레트에서 드래그 앤 드롭 (인프라, 플로차트, 클라우드 아이콘) |
| **노드 이동** | 노드를 드래그 |
...
| **선 타입** | 우측 패널 `선 타입` → 직각 / 직선 / 곡선형 |
| **시작·끝 면 지정** | 우측 패널 `시작 면` / `끝 면` — 노드의 어느 변에서 연결선이 나갈지 고정 |
| **라벨 달기** | 연결선 선택 후 우측 패널 `라벨` 입력 |
| **라벨 위치·방향** | 우측 패널에서 위치를 옮기고, 텍스트 방향을 가로 / 세로 / 선 따라 중에서 선택 |
...
| **PowerPoint** 📽 | 레이어별 슬라이드. 그림이 아니라 편집 가능한 도형·텍스트로 생성됩니다 |
| **PNG** 📷 | 다이어그램 이미지. 1×~4× 배율 선택 (해당 해상도로 다시 그리므로 실제로 선명해집니다) |
| **SVG** ◇ | 벡터 이미지. 확대해도 글자가 깨지지 않고 다른 도구에서 편집할 수 있습니다 |
...
| **공유 링크** 🔗 | 다이어그램 전체를 담은 공유 URL 생성 |

> 내보낸 결과물에는 출처 표기(`FCStudio · https://fc-studio.xyz`)가 들어갑니다. 형식에 맞는 방식으로 한 번씩만 — 이미지는 오른쪽 아래 문구, PDF는 표지와 페이지 하단, PowerPoint는 슬라이드 하단 텍스트, Excel은 시트 행, JSON은 `meta` 필드, IaC 코드는 첫 줄 주석입니다.

#### 이미지 내보내기 (PNG / SVG)

**PNG**를 고르면 해상도를 1×~4× 중에서 선택합니다. 이미 만든 그림을 늘리는 게 아니라 그 해상도로 다시 그리는 것이라 글자가 실제로 선명해집니다. 결과 픽셀 크기가 창에 표시되고, 브라우저 캔버스 한계를 넘는 배율은 선택할 수 없습니다.

**SVG**는 벡터라 배율을 고를 필요가 없습니다. 아이콘과 Mermaid 다이어그램도 벡터로 유지됩니다.

> SVG의 **폰트 포함** 옵션을 켜면 Pretendard 글꼴이 파일 안에 들어가, 그 폰트가 없는 다른 PC에서 열어도 글자 모양과 줄바꿈이 그대로 유지됩니다. 대신 파일이 약 600KB 커집니다. 끄면 28KB 수준이고, 여는 쪽 시스템 글꼴로 대체되어 글자 폭이 조금 달라질 수 있습니다.

PDF와 Excel에 들어가는 다이어그램 그림도 2배 해상도로 넣습니다 — 배치 크기는 그대로이고 인쇄하거나 확대할 때만 차이가 납니다.

#### Mermaid 가져오기

Mermaid 문법으로 된 텍스트를 캔버스에 그냥 **붙여넣기(`Ctrl+V`)** 하면 자동으로 인식하고 두 가지 방법을 제안합니다.

| 선택 | 결과 |
|------|------|
| **📊 도표로 그리기** | Mermaid를 해석해 Loom의 노드와 연결선으로 변환합니다. 변환 후에는 일반 노드와 똑같이 편집·리소스 설정·내보내기가 가능합니다. |
| **📄 Mermaid 패널로 추가** | 원문을 그대로 담은 패널 노드로 추가하고 그래프로 렌더링합니다. 원문을 언제든 고칠 수 있습니다. |

도형 변환은 `flowchart` / `graph` 문법만 지원합니다. sequence, gantt, class 등 다른 다이어그램은 패널로만 추가되며, 이 경우 선택 창이 그 사실을 알려줍니다.

패널 노드를 선택하면 우측 패널에서 **Mermaid 원문**을 편집하고, **그래프로 보기**를 끄면 원문 텍스트를 그대로 볼 수 있습니다. **📊 도형으로 변환** 버튼으로 나중에 도형으로 바꿀 수도 있습니다.

#### 공유 링크

파일 교환 없이 URL 하나로 다이어그램을 공유할 수 있습니다.

1. **Export ▾** → **🔗 공유 링크** 클릭
2. 생성된 URL을 복사하거나 **📧 메일로 보내기** 클릭
3. 받는 사람이 링크를 열면 다이어그램이 자동으로 로드됩니다

> **기술 참고**: 다이어그램 데이터는 LZ-String으로 압축되어 URL의 **해시 프래그먼트**(`#data=...`)에 포함됩니다. 쿼리 파라미터(`?data=...`)가 아닌 해시를 사용하는 이유는, 해시는 클라이언트에서만 처리되고 서버로 전송되지 않아 CDN/서버의 URI 길이 제한(예: CloudFront 414 에러)을 우회할 수 있기 때문입니다. 매우 큰 다이어그램(32,000자 이상)은 브라우저 제한을 초과할 수 있으며, 이 경우 JSON 파일로 공유하세요.
...
- [x] Terraform 코드 내보내기
- [x] URL 링크로 다이어그램 공유
- [x] 서브 인프라 드릴다운 및 리소스 합산
- [x] 드래그 앤 드롭 전용 노드 생성
- [x] PowerPoint(.pptx) 내보내기
- [x] Mermaid 텍스트 가져오기
- [x] SVG 벡터 내보내기 및 PNG 해상도 선택
- [x] 외부 CDN 없이 완전 오프라인 동작
